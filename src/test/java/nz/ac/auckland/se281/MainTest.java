...
public class MainTest {
  ...
  public static class YourTests extends CliTest {
    ...
    public static class YourTask2Tests extends CliTest {
      ...
      // test if creating an operator with extra spaces in name trims correctly
@Test
public void T2_14_create_operator_extra_spaces_in_name() throws Exception {
    runCommands(
        CREATE_OPERATOR,
        "'    Sky Tower Adventures    '",
        "'AKL'",
        EXIT
    );
    assertContains("Successfully created operator 'Sky Tower Adventures'");
    assertDoesNotContain("Successfully created operator '    Sky Tower Adventures    '", true);
}

// test if creating an operator with duplicate name fails
@Test
public void T2_15_create_operator_duplicate_name() throws Exception {
    runCommands(
        CREATE_OPERATOR,
        "'Sky Tower Adventures'",
        "'AKL'",
    ...
    assertContains("Operator not created: the operator name 'Sky Tower Adventures' already exists same location for 'Auckland | Tāmaki Makaurau'.");
    //assertDoesNotContain("Successfully created operator", true);
}

// test if creating an operator with duplicate ID fails (different name, same abbreviation)

// test if searching activities ignores surrounding spaces in keyword
@Test
public void T2_17_search_activities_keyword_with_spaces() throws Exception {
    runCommands(
        unpack(
            CREATE_14_OPERATORS,
            CREATE_27_ACTIVITIES,
...
// test if creating an activity with "Other" type works when given invalid type
@Test
public void T2_18_create_activity_invalid_type_fallsback_other() throws Exception {
    runCommands(
        CREATE_OPERATOR,
        "'Sky Tower Adventures'",
        "'AKL'",
...
// test if searching activities is case-insensitive for location
@Test
public void T2_19_search_activities_location_case_insensitive() throws Exception {
    runCommands(
        unpack(
            CREATE_14_OPERATORS,
            CREATE_27_ACTIVITIES,
    ...
    assertContains("Jumping Through Political Loopholes");
    assertContains("Politics with a View");
    assertDoesNotContain("There are no matching activities found.", true);
}

// test if searching activities partially matches activity name
@Test
public void T2_20_search_activities_partial_name_match() throws Exception {
    runCommands(
        unpack(
            CREATE_14_OPERATORS,
            CREATE_27_ACTIVITIES,
      ...
      public void T3_XX_endorse_public_review() throws Exception {
        runCommands(
            unpack(
                ...
                ADD_PUBLIC_REVIEW,
                "WACT-AKL-001-001",
                options("Alice", "n", "-1", "Could be better"),
                ENDORSE_REVIEW,
                "WACT-AKL-001-001-R1",
                DISPLAY_REVIEWS,
                "WACT-AKL-001-001",
